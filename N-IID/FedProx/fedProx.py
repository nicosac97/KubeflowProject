...
def FedProx(model, training_sets:list, n_iter:int, testing_sets:list, mu=0.3, 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--outputloss', type=str)
    parser.add_argument('--outputaccuracy', type=str)
    ...
    mnist_iid_traindata,args.n_iter ,mnist_iid_testdata,args.mu, args.epochs, 
       args.lr)
    finish=time.time()
    ...
    Path(args.outputloss).parent.mkdir(parents=True, exist_ok=True)
    Path(args.outputaccuracy).parent.mkdir(parents=True, exist_ok=True)



    #saving the loss metric list to the relative outputfile
    with open( args.outputloss, 'w') as lo:
        for _list in loss_hist_FA_iid:
            for _string in _list:
                lo.write(str(_string) + '\n')
      
    #saving the loss metric list to the relative outputfile
    with open( args.outputaccuracy, 'w') as ac:
        for _list in acc_hist_FA_iid:
            for _string in _list:
                ac.write(str(_string) + '\n')
